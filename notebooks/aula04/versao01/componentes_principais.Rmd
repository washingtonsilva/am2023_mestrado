...
variável `Assault` domina o primeiro componente principal, o qual é 
...
mudança de sinal, portanto podemos tentar reproduzir a Fig. 2 fazendo algumas 
pequenas alterações:
...
Após analisar com certo detalhe o resultado produzido pela funcão 
`prcomp`, podemos obter os principais resultados de uma PCA com 
esta funcão fazendo:
...
O pacote `FactoMineR` disponibiliza métodos exploratórios para resumir, visualizar e descrever dados multivariados. Os principais métodos de PCA são disponibilizados, além de outros: 
...
Scree Plot com `factoextra::fviz_screeplot`
...
Mapa de fatores das variáveis com `factoextra::fviz_pca_var`.

```{r}
fviz_pca_var(facto_res, col.var = "orange")
```


Mapa de fatores dos indivíduos com `factoextra::fviz_pca_ind`. 

```{r}
fviz_pca_ind(facto_res, col.var = "blue")
```


Mapa de fatores das variáveis e indivíduos, que é o biplot, com 
`factoextra::fviz_pca_biplot`.

```{r}
fviz_pca_biplot(facto_res, 
                label = "all", 
                col.var = "orange",
                col.ind = "blue",
                repel = TRUE)
