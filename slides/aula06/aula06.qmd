...
4. Escolher uma *rotação* dos fatores.

5. Estimar o modelo e interpretar os fatores resultantes.

6. Repetir as etaps 3-5 se os resultados não forem claros, selecionar o 
modelo mais útil.
...
## `factanal`: Compreendendo os resultados

### `Uniquenesses`

- A primeiro parte fornece as estimativas das "Uniquenesses", 
que variam de 0 a 1. É $\varepsilon$ do modelo estatístico.

- Ficamos atentos $\varepsilon$ altos. Uma alta singularidade para uma variável pode significar que ela não se encaixa perfeitamente em nossos fatores. 

- Por exemplo, `latest` possui $\varepsilon = 0.796$ 
...
métodos de rotação, juntamente com quantidades gigantescas de resultados (A saída da função factanal() é bastante pequena em comparação com outros programas.) 

- No entanto, a decisão mais importante é o número de fatores. A escolha do método de estimação e rotação é menos crucial de acordo com Johnson e Wichern (2007). 
...
- Em uma nota decepcionante, Johnson e Wichern (2007) também afirmam que a "a grande maioria das tentativas de análises fatoriais não produz resultados 
claros" (p. 526). 
